...
function finalizarQ3() {
  ...
  // Nota: franja foi explorada em Q2, não precisa ser re-respondida em Q3
  if (!biotipo || !linhasCorpo || !comprimento) {
    alert('⚠️ Responda as 3 perguntas para continuar');
    return;
  }
  ...
  // CRÍTICO: Armazenar no state global para Q4 validar
  state.corpo.biotipo = biotipo;
  state.corpo.linhasCorpo = linhasCorpo;
  state.corpo.comprimentoPreferido = comprimento;

  // Marcar Q3 como explorado (para futuro tracking)
  q3State.etapa = 'completo';

  // Exibir confirmação brevemente e depois redirecionar para Q4
  const conteudo = document.getElementById('conteudo-q3');
  conteudo.innerHTML = `
    <div style="text-align: center; padding: 40px;">
      ...
      <div style="background: rgba(76, 175, 80, 0.1); border: 1px solid #4CAF50; padding: 25px; border-radius: 12px; text-align: left; margin-bottom: 30px;">
        ...
        <p style="color: #aaa;"><strong>Comprimento Preferido:</strong> <span style="color: #4CAF50;">${comprimento.toUpperCase()}</span></p>
      </div>
      <p style="color: #ccc; line-height: 1.8; margin-bottom: 30px;">
        Você completou os 3 quadrantes! Direcionando para consolidação...
      </p>
      <div style="display: flex; justify-content: center; gap: 6px; margin-bottom: 20px;">
        <div style="width: 8px; height: 8px; background: #4CAF50; border-radius: 50%; animation: pulse 1s infinite;"></div>
        <div style="width: 8px; height: 8px; background: #4CAF50; border-radius: 50%; animation: pulse 1s infinite 0.2s;"></div>
        <div style="width: 8px; height: 8px; background: #4CAF50; border-radius: 50%; animation: pulse 1s infinite 0.4s;"></div>
      </div>
    </div>
    <style>
      @keyframes pulse {
  ...
  // Redirecionar para Q4 após 1.5 segundos
  setTimeout(() => {
    fecharPainel(3);
    abrirQuadrante4();
  }, 1500);
