...
function desenharMascara(landmarks, maskCanvas, encaixado) {
  ...
  // Desenhar diretamente com pontos do MediaPipe (sem escalas)
  function pathSimples(indices, fechar = false) {
  ...
  ctx.strokeStyle = cor;
  ctx.lineWidth = 2.5;
  pathSimples(FACE_OVAL, true);

  // Sobrancelhas
  ctx.lineWidth = 1.5;
  pathSimples(LEFT_BROW, false);
  pathSimples(RIGHT_BROW, false);

  // Olhos
  pathSimples(LEFT_EYE, true);
  pathSimples(RIGHT_EYE, true);

  // Nariz
  ctx.strokeStyle = corFraca;
  ctx.lineWidth = 1.2;
  pathSimples(NOSE_BRIDGE, false);
  pathSimples(NOSE_BASE, false);

  // Boca
  ctx.strokeStyle = cor;
  ctx.lineWidth = 1.5;
  pathSimples(LIPS_OUTER, true);
  pathSimples(LIPS_INNER, true);
