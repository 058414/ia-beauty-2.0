...
function desenharMascara(landmarks, maskCanvas, encaixado) {
  ...
  // NOVO: Calcular altura e largura REAIS do rosto detectado
  ...
  const alturaRosto = maxY - minY;
  const larguraRosto = maxX - minX;
  const cx = (minX + maxX) / 2;
  const cy = (minY + maxY) / 2;

  // NOVO: Calcular escalas DINÂMICAS baseadas no rosto real
  // Objetivo: ~200px de altura, proporção 2.4:1 (altura:largura)
  const alturaDesejada = 200;
  const proporcaoDesejada = 2.4;
  const escalaY = alturaDesejada / alturaRosto;
  const escalaX = escalaY / proporcaoDesejada;
