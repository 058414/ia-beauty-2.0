...
async function iniciarCameraQ1() {
  ...
  try {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      throw new Error('Câmera não disponível neste contexto. Use HTTPS ou acesse via localhost.');
    }

    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Timeout: câmera não respondeu em 10 segundos. Permissão negada ou câmera ocupada?')), 10000)
    );

    cameraState.stream = await Promise.race([
