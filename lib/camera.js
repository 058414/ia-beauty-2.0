...
function desenharMascara(landmarks, maskCanvas, encaixado) {
  ...
  function path(indices, fechar = false) {
  ...
  // Contorno do rosto (EXPANDIDO para testa até queixo)
  ctx.strokeStyle = cor;
  ctx.lineWidth = 2.5;
  ...
  if (ovalPts.length > 2) {
    ...
    // Expandir contorno: menos horizontal (1.15x), mais vertical (1.55x) para cobrir testa-queixo
    const escalaX = 1.15;
    const escalaY = 1.55;
  ...
  // Sobrancelhas
  ctx.lineWidth = 1.5;
  path(LEFT_BROW, false);
  path(RIGHT_BROW, false);
  ...
  ctx.lineWidth = 1.2;
  path(NOSE_BRIDGE, false);
  path(NOSE_BASE, false);

  // Boca
  ctx.strokeStyle = cor;
  ctx.lineWidth = 1.5;
  path(LIPS_OUTER, true);
  path(LIPS_INNER, true);
