...
function desenharMascara(landmarks, maskCanvas, encaixado) {
  ...
  function pathNatural(indices, fechar = false) {
  ...
  // Contorno do rosto (APENAS ISTO EXPANDIDO)
  ctx.strokeStyle = cor;
  ctx.lineWidth = 2.5;
  if (ovalPts.length > 2) {
  ...
  // Sobrancelhas (NATURAL - sincronizadas automaticamente)
  ctx.lineWidth = 1.5;
  pathNatural(LEFT_BROW, false);
  pathNatural(RIGHT_BROW, false);

  // Olhos (NATURAL - sincronizadas automaticamente)
  pathNatural(LEFT_EYE, true);
  pathNatural(RIGHT_EYE, true);

  // Nariz (NATURAL - sincronizado automaticamente)
  ctx.strokeStyle = corFraca;
  ctx.lineWidth = 1.2;
  pathNatural(NOSE_BRIDGE, false);
  pathNatural(NOSE_BASE, false);

  // Boca (NATURAL - sincronizada automaticamente)
  ctx.strokeStyle = cor;
  ctx.lineWidth = 1.5;
  pathNatural(LIPS_OUTER, true);
  pathNatural(LIPS_INNER, true);
