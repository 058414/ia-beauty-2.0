...
function desenharMascara(landmarks, maskCanvas, encaixado) {
  ...
  // DETECTAR ROSTO REAL E CALCULAR PROPORÇÕES
  // 1. Medir altura e largura reais do rosto
  ...
  const alturaReal = maxY - minY;
  const larguraReal = maxX - minX;
  const cxReal = (minX + maxX) / 2;
  const cyReal = (minY + maxY) / 2;

  // 2. Calcular escala para manter proporção 2.4:1 (altura:largura)
  const proporcaoDesejada = 2.4;
  const escalaAltura = 1.2; // Expandir um pouco a altura
  const escalaLargura = escalaAltura / proporcaoDesejada; // Manter proporção

  // 3. Desenhar com escalas calculadas do rosto real
  function pathComEscala(indices, fechar = false) {
    const pts = indices.map(i => p(i)).filter(Boolean).map(pt => ({
      x: cxReal + (pt.x - cxReal) * escalaLargura,
      y: cyReal + (pt.y - cyReal) * escalaAltura
    }));
  ...
  // Contorno do rosto (COM PROPORÇÕES REAIS)
  ctx.strokeStyle = cor;
  ctx.lineWidth = 2.5;
  pathComEscala(FACE_OVAL, true);

  // Sobrancelhas (SINCRONIZADAS com rosto real)
  ctx.lineWidth = 1.5;
  pathComEscala(LEFT_BROW, false);
  pathComEscala(RIGHT_BROW, false);

  // Olhos (SINCRONIZADOS com rosto real)
  pathComEscala(LEFT_EYE, true);
  pathComEscala(RIGHT_EYE, true);

  // Nariz (SINCRONIZADO com rosto real)
  ctx.strokeStyle = corFraca;
  ctx.lineWidth = 1.2;
  pathComEscala(NOSE_BRIDGE, false);
  pathComEscala(NOSE_BASE, false);

  // Boca (SINCRONIZADA com rosto real)
  ctx.strokeStyle = cor;
  ctx.lineWidth = 1.5;
  pathComEscala(LIPS_OUTER, true);
  pathComEscala(LIPS_INNER, true);
