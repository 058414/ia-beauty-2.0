...
function desenharMascara(landmarks, maskCanvas, encaixado) {
  ...
  // NOVO: Calcular escalas DINÂMICAS baseado nas proporções reais do rosto
  ...
  const alturaRosto = Math.max(1, maxY - minY);  // Evitar divisão por zero
  const escalaX = 0.875;  // Mantém padrão se não conseguir calcular
  let escalaY = 2.1;      // Mantém padrão

  // Se conseguir medir altura, calcular escala proporcional
  if (alturaRosto > 50 && alturaRosto < 500) {
    escalaY = Math.max(1.5, Math.min(3.0, (200 / alturaRosto) * 2.1));
  }
  ...
  // Contorno do rosto com ajuste inteligente de testa (hairline)
  ctx.strokeStyle = cor;
  ctx.lineWidth = 2.5;
  if (ovalPts.length > 2 && minY !== Infinity) {
    const pForehead = p(10);   // Meio da testa
    const pChin = p(152);      // Queixo
    const faceH = pChin && pForehead ? (pChin.y - pForehead.y) : (maxY - minY);

    // Ajuste de hairline: 15% acima
    const hairOffset = faceH * 0.15;
    const upperLimit = pForehead && pForehead.y ? pForehead.y + faceH * 0.20 : minY;

    // Expandir pontos
    let adjusted = ovalPts.map(pt => {
      const expanded = { x: cx + (pt.x - cx) * escalaX, y: cy + (pt.y - cy) * escalaY };
      // Ajustar testa apenas se ponto está acima do limite
      if (expanded.y < upperLimit) {
        const ratio = 1 - (expanded.y - (pForehead?.y || minY)) / (upperLimit - (pForehead?.y || minY));
        expanded.y = expanded.y - hairOffset * Math.max(0, Math.min(1, ratio));
      }
      return expanded;
    });
    ...
    // Usar curvas suaves em vez de linhas retas
    ...
    ctx.quadraticCurveTo(adjusted[adjusted.length-1].x, adjusted[adjusted.length-1].y, adjusted[0].x, adjusted[0].y);
    ctx.closePath();
    ctx.stroke();
    ctx.globalAlpha = 1;
  }
