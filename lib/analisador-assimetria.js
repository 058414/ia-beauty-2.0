...
function analisarAssimetria(landmarks) {
  ...
  // IMPORTANTE: A câmera está espelhada (transform: scaleX(-1))
  // Então precisamos inverter: lado esquerdo nos landmarks = lado DIREITO visualmente
  let lado_alto, lado_baixo;

  if (altura_esq < altura_dir) {
    lado_alto = 'direito';        // esq menor Y = mais alto visualmente = lado DIREITO (invertido)
    lado_baixo = 'esquerdo';
  } else if (altura_dir < altura_esq) {
    lado_alto = 'esquerdo';       // dir menor Y = mais alto visualmente = lado ESQUERDO (invertido)
    lado_baixo = 'direito';
  } else {
