...
async function abrirQuadrante1() {
  ...
  setTimeout(async () => {
    try {
      document.getElementById('status-1').innerHTML = '✅ Solicitando acesso à câmera...';
      document.getElementById('status-1').style.opacity = '1';

      await iniciarCameraQ1();

      document.getElementById('status-2').innerHTML = '✅ MediaPipe carregado!';
      document.getElementById('status-2').style.opacity = '1';
      document.getElementById('status-3').innerHTML = '✅ Câmera pronta!';
      ...
      conteudo.innerHTML = `<div style="padding: 40px; text-align: center; color: #ff6b6b;">
        ...
        <p style="color: #aaa; font-size: 0.9rem; margin-top: 15px;">Dica: Em celular via HTTP, câmera não funciona. Use HTTPS.</p>
